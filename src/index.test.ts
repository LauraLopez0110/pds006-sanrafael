...
describe("Elysia Device API (multipart compatible)", () => {
  let app: Elysia;
  let orm: MikroORM; // 🧩 NUEVO: manejador global de MikroORM
  ...
  beforeAll(async () => {
    ...
    console.log("✅ MikroORM initialized and Elysia running on port 3000");
  });

  afterAll(async () => {
    app.stop();
    await orm.close(true); // 🧩 NUEVO: cierra la conexión a la base de datos
